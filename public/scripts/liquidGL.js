...
(() => {
  ...
  /* --------------------------------------------------
   *  Color-space patch for html2canvas
   *  html2canvas 1.4.1 cannot parse oklch/oklab/lab/lch.
   *  We wrap getComputedStyle during capture so every
   *  color value html2canvas reads is converted to sRGB.
   * ------------------------------------------------*/
  const _modernColorRe = /\b(oklch|oklab|lab|lch)\s*\(/i;
  const _modernColorGRe = /\b(oklch|oklab|lab|lch)\([^)]+\)/gi;
  const _colorCache = new Map();
  let _cvs, _cctx;

  function _convertColor(match) {
    if (_colorCache.has(match)) return _colorCache.get(match);
    if (!_cvs) { _cvs = document.createElement("canvas"); _cvs.width = _cvs.height = 1; _cctx = _cvs.getContext("2d"); }
    _cctx.clearRect(0, 0, 1, 1);
    _cctx.fillStyle = "rgba(0,0,0,0)";
    _cctx.fillStyle = match;
    _cctx.fillRect(0, 0, 1, 1);
    const d = _cctx.getImageData(0, 0, 1, 1).data;
    const rgb = d[3] === 255
      ? "rgb(" + d[0] + ", " + d[1] + ", " + d[2] + ")"
      : "rgba(" + d[0] + ", " + d[1] + ", " + d[2] + ", " + (d[3] / 255).toFixed(4) + ")";
    _colorCache.set(match, rgb);
    return rgb;
  }

  function _sanitizeValue(val) {
    if (typeof val !== "string" || !_modernColorRe.test(val)) return val;
    return val.replace(_modernColorGRe, _convertColor);
  }

  function _wrapGetComputedStyle() {
    const orig = window.getComputedStyle;
    window.getComputedStyle = function () {
      const style = orig.apply(window, arguments);
      return new Proxy(style, {
        get(target, prop) {
          if (prop === "getPropertyValue") {
            return function (name) { return _sanitizeValue(target.getPropertyValue(name)); };
          }
          if (typeof prop === "symbol" || prop === "length" || prop === "parentRule") {
            return Reflect.get(target, prop);
          }
          const val = Reflect.get(target, prop);
          if (typeof val === "function") return val.bind(target);
          if (typeof val === "string") return _sanitizeValue(val);
          return val;
        }
      });
    };
  ...
  function _unwrapGetComputedStyle(orig) {
  ...
  class liquidGLRenderer {
    ...
    async captureSnapshot() {
      ...
      /* Wrap getComputedStyle to convert modern color functions to sRGB */
      const origGCS = _wrapGetComputedStyle();
      const undos = [() => _unwrapGetComputedStyle(origGCS)];
    ...
    _updateDynamicVideos() {
      ...
      this._videoNodes = this._videoNodes.filter((vid) => {
        if (effectiveZ(vid) >= maxLensZ) return true;
        if (this._isIgnored(vid) || vid.readyState < 2) return true;
        ...
        try {
          ...
          /* CORS or tainted canvas — drop this video permanently */
          console.warn("liquidGL: Skipping cross-origin video", vid.src);
          return false;
        }
      });
    }
    ...
    _updateDynamicNodes() {
      ...
      this._dynamicNodes.forEach((node) => {
        ...
        if (meta.needsRecapture && !meta._capturing && !this._isScrolling) {
          ...
          const dynOrigGCS = _wrapGetComputedStyle();
          html2canvas(el, {
            backgroundColor: null,
            ...
            .finally(() => {
              _unwrapGetComputedStyle(dynOrigGCS);
